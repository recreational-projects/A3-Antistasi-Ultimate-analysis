...
def get_military_zone_markers(mission_sqm_path: Path) -> dict[str, list[Marker]]:
    """Get relevant `Marker`s from the file."""
    with mission_sqm_path.open(errors="ignore") as f:
        data = f.read()

    try:
        mission = armaclass.parse(data)
        log_msg = f"Parsed `{mission_sqm_path}`."
        LOGGER.debug(log_msg)
    except armaclass.ParseError:
        log_msg = f"Couldn't parse `{mission_sqm_path}`; may be binarized."
